...
// --- Lê os pinos analógicos
  for(byte Sensor=0;Sensor<NPin;Sensor++)
  {
   ...
   {
     // DUMMY READ: Leitura descartada para resolver o "ADC Crosstalk" (Ghosting).
     // O sensor do Hihat satura o capacitor do ADC. Quando o Arduino pula 
     // imediatamente para a próxima porta (ex: A1), o capacitor não teve tempo 
     // de descarregar e gera um acionamento falso na porta adjacente.
     analogRead(Sensor); 
   ...
   if (Sensor == 10 || Sensor == 11) {
       if (Pin[Sensor].State == Piezo_Time) {
           byte other = (Sensor == 10) ? 11 : 10;
           if (Pin[other].State != Normal_Time && Pin[other].MaxReading > Pin[Sensor].MaxReading) {
               Pin[Sensor].State = Normal_Time;
           }
       }
   }
  ...
  for(byte i=0;i<(NSensor*8);i++)
  {
   if (i == 10 || i == 11) {
       if (Pin[i].State == Piezo_Time) {
           byte other = (i == 10) ? 11 : 10;
           if (Pin[other].State != Normal_Time && Pin[other].MaxReading > Pin[i].MaxReading) {
