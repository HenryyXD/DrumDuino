...
#define MAX_CURVE_INPUT 1023 // lookup tables cover 8 segments of 128 (0..1023)
...
//      yn_0 = 0.5 + ((float)analogRead(sensor)*(float)Gain)/64.0;
      int raw_val = ANALOGREAD(sensor,pin);
      yn_0 = 0.5 + ((float)raw_val*(float)Gain)/64.0;
      if (yn_0 > MAX_CURVE_INPUT) yn_0 = MAX_CURVE_INPUT;
      ...
      else if(State==Scan_Time) 
      {
        if ((GlobalTime-Time) < ScanTime)
        {
          ...
          {
            MaxReading = yn_0;
            rawMaxReading = raw_val; // Save the raw value for A10/A11 comparison
  ...
  void playMIDI(byte i,pin* dual)
  {
    ...
    {             //Piezo ou HH
      if(Type==Piezo || Type==HH)
      {
        ...
        // As requested: Cupula (A11) will ALWAYS output 100% volume (127) when triggered
        if (i == 11) {
            v = 127;
        }
        ...
        // Strict Dual Suppression: If this pad won and played, suppress its dual pad
        if (XtalkGroup > 0 && dual->Type != Disabled && dual->XtalkGroup == XtalkGroup) {
            dual->State = Mask_Time;
            dual->MaxReading = -1;
            dual->Time = GlobalTime;
        }
  ...
  byte useCurve()
  {
    ...
    if (reading > MAX_CURVE_INPUT) reading = MAX_CURVE_INPUT;
    if (reading < 0) reading = 0;
    float f=((float)CurveForm)/32.0;//[1;127]->[0.;4.0]
    
    if(Curve==Linear)
    {
      ret=0.5 + ((float)reading*f/8.0);
    }
    else
    {
      int i=reading/128;
